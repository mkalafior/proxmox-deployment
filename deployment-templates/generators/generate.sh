...
create_deployment_config() {
    ...
    if [[ -f "$DEPLOYMENT_DIR/service-config.yml" && "$FORCE_OVERWRITE" != "true" ]]; then
        ...
        local temp_vars_file="$TARGET_PROJECT_ROOT/.secure_temp/service-vars-$$.yml"
        mkdir -p "$(dirname "$temp_vars_file")" && chmod 700 "$(dirname "$temp_vars_file")"
...
# Run Ansible playbook from project root (where ansible.cfg is located)
echo "🚀 Deploying service..."
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SERVICE_NAME="$(basename "$SCRIPT_DIR")"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
cd "$PROJECT_ROOT"
ansible-playbook -i localhost, "deployments/${SERVICE_NAME}/deploy.yml"
EOF
